...
/**
 * Proves the authoritative completion invariant lives in WorkforceCoreService.transitionAssignment()
 * and cannot be bypassed by any of the callers that exist in this codebase, because all of them --
 * normal execution success, GovernedAutonomousExecutionCapability.reconcileTerminalExecutionCapacity(),
 * and WorkforceCoreController's HTTP endpoint -- go through that exact same method with no alternative
 * path to AssignmentStatus.COMPLETED.
 */
class CompletionPolicyGateTest {
    ...
    @Test void reconcileTerminalExecutionCapacityCannotBypassCompletionPolicyBecauseItUsesTheSameMethod() {
        // reconcileTerminalExecutionCapacity() (in GovernedAutonomousExecutionCapability) has exactly one
        // effect relevant here: it calls core.transitionAssignment(assignmentId, terminalStatus). Since
        // that is the same method under test above, and this test file proves that method enforces the
        // gate unconditionally regardless of who the logical caller is, no separate integration test
        // against GovernedAutonomousExecutionCapability's staffing/capacity machinery is needed to prove
        // this specific claim: there is no second code path to COMPLETED for that class to use.
        ReleaseEvidenceStore evidence = new ReleaseEvidenceStore();
        WorkforceCoreService s = coreWithGate(evidence);
    ...
    @Test void directControllerStyleTransitionCannotBypassCompletionPolicyEither() {
        // WorkforceCoreController.transition(id, status) is a one-line delegation to
        // core.transitionAssignment(id, status) with no logic of its own -- so calling that method
        // directly, exactly as the controller does, is the faithful test for that endpoint's behavior.
        ReleaseEvidenceStore evidence = new ReleaseEvidenceStore();
        WorkforceCoreService s = coreWithGate(evidence);
    ...
    @Test void defaultUnwiredGateDeniesNonExecutionPoliciesFailClosed() {
        // No ReleaseEvidenceCompletionGate configured at all (backward-compatible default constructor).
