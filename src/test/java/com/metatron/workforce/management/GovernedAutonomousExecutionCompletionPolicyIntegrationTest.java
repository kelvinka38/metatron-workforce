...
class GovernedAutonomousExecutionCompletionPolicyIntegrationTest {
    ...
    @Test
    void multiStepProductionRequiredObjectiveOnlyTheReleaseOwnerStepRequiresReleaseEvidence() {
        ReleaseEvidenceStore evidence = new ReleaseEvidenceStore();
        WorkforceCoreService core = coreWithGate(evidence);
        GovernedAutonomousExecutionCapability governed = new GovernedAutonomousExecutionCapability(
                successfulDelegate(), core, new ExecutionAdmissionService(), CLOCK, Duration.ofSeconds(1));

        com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec prerequisite =
                new com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec(
                        "step-1", "run tests", "target", "test.capability", List.of(),
                        com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec.Consequence.READ_ONLY);
        com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec build =
                new com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec(
                        "step-2", "build artifact", "target", "test.capability", List.of(),
                        com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec.Consequence.READ_ONLY);
        com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec releaseOwner =
                new com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec(
                        "step-3", "publish release", "target", "test.capability", List.of(),
                        com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec.Consequence.READ_ONLY);
        ...
        com.metatron.workforce.interaction.intelligence.NormalizedRequest objective =
                new com.metatron.workforce.interaction.intelligence.NormalizedRequest(
                        ...
                        null, null, "")
                .withCompletionPolicy(CompletionPolicy.PRODUCTION_REQUIRED)
                .withExecutionWorkPlan(List.of(prerequisite, build, releaseOwner));

        List<com.metatron.workforce.interaction.intelligence.ExecutionWorkSpec> plan = objective.executionWorkPlan();
        ...
        // Prerequisite steps complete normally, no release evidence required at all.
        governed.execute(new AutonomousExecutionCapability.CapabilityRequest("human:primary", "org-metatron", "objective-multi", plan.get(0)));
        governed.execute(new AutonomousExecutionCapability.CapabilityRequest("human:primary", "org-metatron", "objective-multi", plan.get(1)));
        List<WorkforceCoreService.Assignment> afterPrereqs = core.allAssignments();
        assertEquals(2, afterPrereqs.size());
        assertTrue(afterPrereqs.stream().allMatch(a -> a.status() == WorkforceCoreService.AssignmentStatus.COMPLETED),
                "ordinary prerequisite steps must complete without any release evidence");

        // The release-owner step executes successfully but stays ACTIVE pending release evidence.
        governed.execute(new AutonomousExecutionCapability.CapabilityRequest("human:primary", "org-metatron", "objective-multi", plan.get(2)));
        WorkforceCoreService.Assignment releaseAssignment = core.allAssignments().stream()
                .filter(a -> a.completionPolicy() == CompletionPolicy.PRODUCTION_REQUIRED).findFirst().orElseThrow();
        assertEquals(WorkforceCoreService.AssignmentStatus.ACTIVE, releaseAssignment.status());
        assertThrows(IllegalStateException.class,
                () -> core.transitionAssignment(releaseAssignment.assignmentId(), WorkforceCoreService.AssignmentStatus.COMPLETED));

        // No duplicate evidence was ever required for the prerequisite/build steps' assignments.
        for (WorkforceCoreService.Assignment a : afterPrereqs) {
            assertNull(evidence.get(a.assignmentId()), "ordinary steps must never need their own release-evidence record");
        }

        // Full release chain against the release-owner assignment only.
        String id = releaseAssignment.assignmentId();
        evidence.recordPrPublished(id, "objective-multi", "kelvinka38/metatron-workforce", 456, "headSha", "main", "baseSha", "planner");
        evidence.recordMerge(id, "mergeSha", "release-control");
        evidence.recordDeployment(id, "mergeSha", "digest", "release-control");
        evidence.recordVerification(id, "mergeSha", "mergeSha", ReleaseEvidence.VerificationStatus.PASSED, "release-control");
