...
class Phase10PrimaryVerticalSliceAcceptanceTest {
    ...
    void completePrimarySliceRunsFromHumanRequestToNextCycle() {
        VerticalSliceRequest request = new VerticalSliceRequest(
                "REQ-001", "HUMAN-001", "HEAD-WORKFORCE", "HEAD-FARM",
        ...
        VerticalSliceResult result = service.run(
                ...
                160, 8, 24,
                100_000, 2_000_000,
                100, 110, 17_000_000, 30_000_000, 31_000_000,
        ...
        assertEquals("HEAD-FARM", result.assignment().toWorkerId());
        ...
        assertEquals(30_000_000, result.economicEvidence().plannedRevenueEvidence());
        assertEquals(31_000_000, result.economicEvidence().actualRevenueEvidence());
        assertTrue(result.economicEvidence().actualContributionEvidence()
                > result.economicEvidence().plannedContributionEvidence());
        assertTrue(result.economicEvidence().authorityBoundary().contains("Economy remains authoritative"));
    ...
    void capacityDeficitBlocksExecutionInsteadOfManufacturingSuccess() {
        FarmOperatingPlan plan = service.plan("PLAN-BLOCKED", "FARM-001",
                200, 4, 24, 100_000, 1_000_000, 100,
                "4 workers x 24h", "understaffed", "test-plan");
        Approval approval = service.approve(plan, "AUTHORITY:HEAD-WORKFORCE", "test-approval");
        ExecutionOutcome outcome = service.execute(plan, approval, 200, 20_000_000, 100, "test-execution");
    ...
    void missingAuthorityCannotProduceApproval() {
        FarmOperatingPlan plan = service.plan("PLAN-AUTH", "FARM-001",
                10, 1, 24, 100_000, 0, 10,
    ...
    void failedExecutionCannotBecomeSuccessfulLearning() {
        FarmOperatingPlan plan = service.plan("PLAN-FAIL", "FARM-001",
                10, 1, 24, 100_000, 0, 100,
                "one shift", "none", "test-plan");
        Approval approval = service.approve(plan, "AUTHORITY:HEAD-WORKFORCE", "test-approval");
        ExecutionOutcome outcome = service.execute(plan, approval, 0, 0, 0, "test-execution");
        CycleReport report = service.report(plan, outcome, "test-report");
        LearningImprovement learning = service.learn(report, "execution-failure",
                "do not adopt failed execution", true, "test-learning");
    ...
    void provenanceIsRequiredAcrossMaterialStages() {
        FarmOperatingPlan plan = service.plan("PLAN-PROV", "FARM-001",
                10, 1, 24, 100_000, 0, 10,
                "one shift", "none", "request=REQ-PROV");
        Approval approval = service.approve(plan, "AUTHORITY:HEAD-WORKFORCE", "request=REQ-PROV");
        ExecutionOutcome execution = service.execute(plan, approval, 10, 1_000_000, 10,
                "approval=AUTHORITY:HEAD-WORKFORCE");
