...
/**
 * Repository audit worker backed by a real GitHub repository read.
 *
 * A PASS is emitted only after GitHub returns repository metadata and a concrete
 * default-branch commit SHA. Network/API failures are surfaced as FAILED; they
 * are never converted into evidence.
 */
public final class RepositoryAuditWorker implements Worker {
    ...
    public RepositoryAuditWorker() {
        this(HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(5)).build(), "https://api.github.com");
    }
    ...
    public WorkerResult execute(WorkerContext context) {
        ...
        try {
            ...
            String evidence = """
                    ...
                    observedAt=%s
                    verdict=PASS
                    """.formatted(context.taskId(), context.objective(), target, defaultBranch, commitSha,
                    repository.statusCode(), commit.statusCode(), completedAt);
