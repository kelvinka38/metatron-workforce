...
public final class Phase10PrimaryVerticalSliceService {
    ...
    public FarmOperatingPlan plan(
        ...
        return new FarmOperatingPlan(
                planId, farmId, requiredLaborHours, available, deficit,
                workerCount, shiftHoursPerWorker, laborCost, resourceCost,
                expectedOutput, schedule, risks, provenance);
    }
    ...
    public ExecutionOutcome execute(
        ...
        if (approval.status() != Approval.Status.APPROVED) {
            return new ExecutionOutcome(
                    "execution-" + plan.planId(), false, 0, 0, 0,
        ...
        return new ExecutionOutcome(
                "execution-" + plan.planId(), true, actualLaborHours,
    ...
    public CycleReport report(FarmOperatingPlan plan, ExecutionOutcome execution, String provenance) {
        ...
        return new CycleReport(
                "report-" + plan.planId(),
                plan.expectedOutput(), execution.actualOutput(), outputVariance,
                plannedCost, execution.actualCost(), costVariance,
                plan.requiredLaborHours(), execution.actualLaborHours(), laborVariance,
                performance, issues, provenance);
    }

    public LearningImprovement learn(CycleReport report, String rootCause, String lesson, boolean validationEvidence, String provenance) {
        ...
        double improved = validated
                ? Math.max(report.plannedOutput(), report.actualOutput())
                : report.plannedOutput();
        return new LearningImprovement(
                "improvement-" + report.reportId(),
                report.plannedOutput(), improved,
                improved - report.plannedOutput(), rootCause, lesson, validated, provenance);
    }
    ...
    public FarmOperatingPlan applyValidatedImprovement(FarmOperatingPlan current, LearningImprovement improvement) {
        ...
        return new FarmOperatingPlan(
                current.planId() + "-next",
                current.farmId(),
                current.requiredLaborHours(),
                current.availableLaborHours(),
                current.capacityDeficitHours(),
                current.workerCount(),
                current.shiftHoursPerWorker(),
                current.plannedLaborCost(),
                current.plannedResourceCost(),
                improvement.improvedOutput(),
                current.schedule(),
                current.risks(),
                current.provenance() + ";validated-improvement=" + improvement.candidateId());
    }

    public VerticalSliceResult run(
        ...
        FarmOperatingPlan plan = plan(
                "plan-" + request.requestId(), farmId, requiredLaborHours,
                workerCount, shiftHoursPerWorker, hourlyLaborCost, resourceCost,
                expectedOutput, schedule, risks, "request=" + request.requestId());
        Approval approval = approve(plan,
                "AUTHORITY:" + request.headOfWorkforceId(),
                "request=" + request.requestId());
        ExecutionOutcome execution = execute(
                plan, approval, plan.requiredLaborHours(), actualCost, actualOutput,
                "plan=" + plan.planId() + ";authority=" + approval.authorityReference());
        CycleReport report = report(plan, execution, "execution=" + execution.executionId());
        LearningImprovement learning = learn(
                report,
        ...
        return new VerticalSliceResult(request, plan, approval, execution, report, learning, nextPlan, "PENDING_HUMAN_REVIEW");
    }
    ...
    public VerticalSliceResult review(VerticalSliceResult result, String decision) {
        ...
        return new VerticalSliceResult(
                result.request(), result.plan(), result.approval(), result.execution(),
