...
final class ObjectiveCompletionPolicyResolver {
    ...
    static CompletionPolicy resolve(NormalizedRequest request) {
        ...
        // A prohibition ("do not merge/deploy") is a ceiling, never by itself a deliverable requirement:
        // it can only rule PRODUCTION_REQUIRED out, never rule PR_REQUIRED in. Only an explicit PR/
        // deliverable signal creates PR_REQUIRED. Contradictory signals (explicit production request AND
        // explicit no-deploy) fail conservatively -- never to PRODUCTION_REQUIRED.
