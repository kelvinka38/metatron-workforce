...
public final class DirectCodingIngressController {
    ...
    public ResponseEntity<?> action(
            @RequestHeader("X-Metatron-Direct-Client") String client,
            @RequestBody DirectCodingIngressService.DirectCodingCommand command,
            HttpServletRequest request) {
        if (!trustedDirectNetworkRequest(request)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new DirectCodingError(false, "direct_coding_private_network_required", ""));
        }
        try {
            return ResponseEntity.ok(service.execute(client, command));
        } catch (GovernanceDeniedException denied) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(new DirectCodingError(false, denied.code(), bounded(denied.getMessage())));
        } catch (SecurityException denied) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(new DirectCodingError(false, bounded(denied.getMessage()), ""));
        } catch (IllegalArgumentException invalid) {
            return ResponseEntity.badRequest()
                    .body(new DirectCodingError(false, "direct_coding_invalid_request", bounded(invalid.getMessage())));
        } catch (IllegalStateException conflict) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(new DirectCodingError(false, bounded(conflict.getMessage()), ""));
        } catch (RuntimeException failure) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new DirectCodingError(false, "direct_coding_failed", failure.getClass().getSimpleName()));
        }
    }
    ...
    public record DirectCodingError(boolean ok, String error, String detail) {
        public DirectCodingError {
