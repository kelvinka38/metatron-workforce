...
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#081018"><title>Metatron · Realtime Control Room</title>
<style>
...
<div id="login" class="wrap login"><div class="card"><div class="brand">METATRON CONTROL ROOM</div><div class="sub">Founder Realtime Dashboard · Workforce execution, performance and evidence</div><p class="meta">Login is delivered only to the authorized Telegram account.</p><button class="btn" onclick="requestCode()">Send login code to Telegram</button><input id="code" inputmode="numeric" maxlength="6" placeholder="000000" autocomplete="one-time-code"><button class="btn secondary" onclick="verify()">Open dashboard</button><div id="loginError" class="error"></div></div></div>
<div id="app" class="wrap hidden"><div class="top"><div><div class="brand">METATRON CONTROL ROOM</div><div id="revision" class="sub"></div></div><div><div class="live"><i class="dot"></i>PRODUCTION LIVE</div><button class="refresh" onclick="load()">Refresh</button></div></div>
<div id="stats" class="grid stats"></div><div class="grid two"><div class="card"><h2>Realtime Objectives</h2><div id="objectives" class="list"></div></div><div class="card"><h2>Alerts / Exceptions</h2><div id="alerts" class="list"></div></div></div>
...
function render(d){$('revision').textContent=`${d.environment} · ${d.revision.slice(0,12)}`;let s=d.summary;let stats=[
['Workers',`${s.activeWorkers}/${s.workers}`],
['Objectives',`${s.activeObjectives}/${s.objectives}`],
['Working',s.executingObjectives],
['Stale / unproven',s.staleObjectives],
['Completed',s.completedObjectives],
['Alerts',s.alerts]
];$('stats').innerHTML=stats.map(x=>`<div class="card"><div class="k">${x[0]}</div><div class="v">${x[1]}</div></div>`).join('');
$('objectives').innerHTML=d.objectivePulse.length?d.objectivePulse.map(o=>{
let meta=`${o.progressPercent}% · ${o.completedWork}/${o.totalWork} work · owner ${o.ownerWorkerId} · staffing ${o.staffingState} · evidence ${o.evidenceCount} · last ${new Date(o.lastActivityAt).toLocaleTimeString()}`;
if(o.blocker) meta+=' · BLOCKER '+o.blocker;
return row(o.summary,meta,o.executionState)
}).join(''):empty('No objectives yet');
...
$('events').innerHTML=d.recentEvents.length?d.recentEvents.slice(0,30).map(e=>row(e.type,`${e.actorWorkerId} · ${e.objectiveId} · ${e.detail}`,e.type.includes('BLOCK')||e.type.includes('ESCALAT')?'BLOCKED':'ACTIVE')).join(''):empty('No management activity yet');$('stamp').textContent='Recorded management state · realtime refresh 2s · '+new Date(d.generatedAt).toLocaleString();}
(async()=>{if(token){let r=await fetch('/workplace/api/auth/status',{headers:{Authorization:'Bearer '+token}});let j=await r.json();if(j.authenticated){showApp();load();setInterval(load,2000)}}})();
</script></body></html>
""";
}
