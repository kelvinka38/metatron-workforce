...
@media(max-width:1200px){.kpis{grid-template-columns:repeat(4,minmax(0,1fr))}.main{grid-template-columns:1fr}.sideStack{grid-template-columns:1fr 1fr}.detailGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:1050px){.main{grid-template-columns:1fr}.sideStack{grid-template-columns:1fr 1fr}.detailGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
function openObjective(id,quiet=false){
  ...
  let steps=o.workItems.length?o.workItems.map((w,i)=>'<div class="step"><div class="stepTop"><div class="stepName">'+(i+1)+'. '+esc(w.objective)+'</div>'+badge(w.state)+'</div><div class="stepMeta"><b>Performer:</b> '+esc(w.performer)+'<br><b>Effect:</b> '+esc(w.effectState)+' · '+w.actionCount+' action(s) · latest '+esc(w.latestAction)+'<br><b>Capability:</b> '+esc(w.requiredCapability)+'<br><b>Target:</b> '+esc(w.target||'—')+'<br><b>Depends:</b> '+esc(w.dependsOn.join(', ')||'none')+'<br><b>DoD:</b> '+esc(w.acceptanceCriteria.join('; ')||'NOT DEFINED')+'<br><b>Evidence required:</b> '+esc(w.evidenceRequirements.join('; ')||'NOT DEFINED')+'</div></div>').join(''):'<div class="muted">Planning has not produced work items yet.</div>';
  ...
  let actions=o.actionRecords.length?o.actionRecords.map(a=>{
    let input=Object.entries(a.inputs||{}).map(([k,v])=>esc(k)+'='+esc(v)).join('<br>')||'none';
    let output=Object.entries(a.outputs||{}).map(([k,v])=>esc(k)+'='+esc(v)).join('<br>')||'none';
    return '<div class="step"><div class="stepTop"><div class="stepName">'+fmtTime(a.recordedAt)+' · '+esc(a.workerId)+' · '+esc(a.actionRef)+'</div>'+badge((a.success?'SUCCESS ':'FAILED ')+a.consequence)+'</div><div class="stepMeta"><b>Step:</b> '+esc(a.workStepId)+' · <b>Cycle:</b> '+a.cycle+'<br><b>Summary:</b> '+esc(a.summary)+'<br><b>Inputs</b><br>'+input+'<br><b>Outputs</b><br>'+output+'<br><b>Reflection:</b> '+esc(a.reflection)+' · '+esc(a.reflectionSummary)+'</div></div>'
  }).join(''):'<div class="muted">NO DURABLE WORKER ACTION RECORDS. Execution claim is not proven at action level.</div>';
  $('drawerBody').innerHTML='<div class="detailGrid">'+boxes+'</div><div class="section"><h3>Work breakdown · actual performer / effect</h3>'+steps+'</div><div class="section"><h3>Worker action log · what actually ran</h3>'+actions+'</div><div class="section"><h3>Execution evidence</h3>'+evidence+'</div><div class="section"><h3>Management activity history</h3>'+ev+'</div>';
