...
public class ReleaseControlService {
     ...
     * identifies THAT SAME recorded PR/head (not a caller-substituted PR/repository), then re-fetches
     * live PR state and compares it to {@code authorization.expectedHeadSha()} before ever invoking the
     * {@link MergeExecutor}. Blocks on missing/mismatched recorded evidence, a stale head, failed
     * required checks, or unsatisfied branch protection -- never bypasses any of them. This is what
     * prevents: record PR A's evidence, submit a MergeAuthorization for PR B, merge B, and have B's
     * merge SHA recorded against A.
     */
