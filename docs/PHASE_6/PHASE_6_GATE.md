...
The executable acceptance evidence is `Phase6AuthorizationExecutionAcceptanceTest` plus Phase 6 regression, full regression, and `bootJar` checks in `.github/workflows/phase6.yml`.

The gate remains non-PASS until a fresh CI run proves these checks on the exact current `main` gate commit.

**GATE DECISION: PENDING FRESH MAIN CI.**

Only a green CI run on the exact current gate commit may change this decision to **PASS** and permit Phase 7 — Reporting / Performance / Economic Evidence.
