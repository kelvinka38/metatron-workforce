...
= NOT EXPOSED to this client, consistent with invariant
  workerReleaseSelfGrant=false in coding-capability-v1.json
```

Conclusion: the intentional design already matches the target architecture — raw
shell/git push is deliberately unwired (`rawRootShellCanonical=false`,
`repositoryCredentialsRemainInWorkforce=true`), and the only sanctioned Worker/direct-MCP
write path is `repository_patch` → `repository_pr_publish`, which does not require a
local git remote because credentials and push logic stay server-side in Workforce.
No new capability was required to satisfy this path for an external Claude MCP client;
it already existed and is contract-tested.
