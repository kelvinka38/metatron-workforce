...
Gateway Head execution-bridge closure additionally requires the deployed runtime to preserve governed `Worker -> Work -> Knowledge -> Gateway audit` execution semantics and fail closed when canonical private-repository knowledge credentials or delegated mutation authority are absent. Temporary self-hosted production workflows used to instantiate and diagnose the first Gateway Head worker have been removed before this certification line.
