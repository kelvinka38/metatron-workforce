...
PY
SINK_PID=$!
PROJECT_A="highway-${GITHUB_RUN_ID}-lane-a"
PROJECT_B="highway-${GITHUB_RUN_ID}-lane-b"
...
cleanup() {
  set +e
  down_lane "$PROJECT_A" "$PORT_A" lane-a
  down_lane "$PROJECT_B" "$PORT_B" lane-b
...
start_lane() {
  ...
  cid=$(METATRON_HOST_PORT="$port" METATRON_STATE_VOLUME_NAME="${project}-state" METATRON_WORKFORCE_NETWORK_NAME="${project}-network" \
    METATRON_WORKFORCE_GATEWAY_ALIAS="workforce-${lane}-${GITHUB_RUN_ID}" docker compose -p "$project" --env-file "$BASE/.env" -f "$COMPOSE" ps -q workforce)
  test -n "$cid"
...
PY
    ...
    case "$state" in COMPLETED|DELIVERED) return 0;; BLOCKED|ESCALATED|CANCELLED) return 2;; esac
    sleep 2
  done
  return 1
}
...
(set +e; live_gateway >"$OUT/live-gateway.log" 2>&1; echo $? >"$OUT/live-gateway.rc") & P1=$!
(set +e; live_observability >"$OUT/live-observability.log" 2>&1; echo $? >"$OUT/live-observability.rc") & P2=$!
(set +e; crash_lane "$PROJECT_A" lane-a "$PORT_A" kelvinka38/bios "${BASE_ID}31" >"$OUT/lane-a.log" 2>&1; echo $? >"$OUT/lane-a.rc") & P3=$!
(set +e; crash_lane "$PROJECT_B" lane-b "$PORT_B" kelvinka38/universal "${BASE_ID}32" >"$OUT/lane-b.log" 2>&1; echo $? >"$OUT/lane-b.rc") & P4=$!
...
for lane in live-gateway live-observability lane-a lane-b; do
  echo "===== $lane ====="; cat "$OUT/$lane.log" || true
...
test -n "$FINAL_LIVE_CID"; test "$(docker inspect "$FINAL_LIVE_CID" --format '{{.State.Health.Status}}')" = healthy
