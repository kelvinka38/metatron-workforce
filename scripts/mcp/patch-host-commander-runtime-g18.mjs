...
// Keep Commander mutation annotations conservative. Mutation tools default to
// destructiveHint=true and idempotentHint=false unless proven otherwise.
NON_DESTRUCTIVE_WRITE_TOOLS.add("commander_open");
...
register("commander_runtime_identity","Prove the running Host Commander MCP generation, immutable runtime image and canonical source SHA.",objectSchema({session_id:{type:"string"},fencing_token:{type:"string"}},["session_id","fencing_token"]),async(a,request)=>{assertKeys(a,["session_id","fencing_token"],["session_id","fencing_token"]);return commanderBrokerResult(request,"commander_runtime_identity",commanderSessionArgs(a,request),45000);});
