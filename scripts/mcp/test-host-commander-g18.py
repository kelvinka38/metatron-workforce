...
with tempfile.TemporaryDirectory(prefix="metatron-g18-test-") as tmp:
    ...
    assert 'NON_DESTRUCTIVE_WRITE_TOOLS.add("commander_open")' in runtime_text
    assert 'IDEMPOTENT_WRITE_TOOLS.add("commander_' not in runtime_text

    release_text = (ROOT / "release-host-commander-g18.sh").read_text(encoding="utf-8")
    for required in (
        'metatron.mcp.source_sha=$EXPECTED_WORKFORCE_SHA',
        'metatron.mcp.release=g18-host-commander',
        'commander_runtime_identity',
        'MCP_G18_RUNTIME_IDENTITY=PASS',
    ):
