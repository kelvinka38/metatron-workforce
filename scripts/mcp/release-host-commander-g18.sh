...
if docker image inspect "$TARGET_IMAGE" >/dev/null 2>&1; then
  ...
  [[ "$EXISTING_SOURCE_SHA" =~ ^[0-9a-f]{40}$ ]] || { echo 'MCP_G18_EXISTING_IMAGE_SOURCE_LABEL_INVALID' >&2; exit 7; }
  [ "$EXISTING_RELEASE" = g18-host-commander ] || { echo 'MCP_G18_EXISTING_IMAGE_RELEASE_LABEL_INVALID' >&2; exit 7; }
  echo "MCP_G18_IMAGE_ALREADY_PRESENT=PASS source_sha=$EXISTING_SOURCE_SHA"
else
...
DOCKER
  docker build --pull=false \
    ...
    --label "metatron.mcp.release=g18-host-commander" \
    -t "$TARGET_IMAGE" "$TMP"
...
[[ "$TARGET_SOURCE_SHA" =~ ^[0-9a-f]{40}$ ]] || { echo 'MCP_G18_IMAGE_SOURCE_LABEL_INVALID' >&2; exit 7; }
[ "$TARGET_RELEASE" = g18-host-commander ] || { echo 'MCP_G18_IMAGE_RELEASE_LABEL_INVALID' >&2; exit 7; }
echo "MCP_G18_IMAGE_ID=$TARGET_IMAGE_ID source_sha=$TARGET_SOURCE_SHA release=$TARGET_RELEASE"
...
grep -q 'def op_commander_runtime_identity(a):' "$TMP/broker.g18.py"
...
IDENTITY_OPEN="$(broker_call "{\"op\":\"commander_open\",\"args\":{\"principal\":\"founder\",\"client_binding\":\"$BINDING\",\"purpose\":\"g18-runtime-identity\",\"scope\":\"read_only\",\"ttl_seconds\":300}}")"
...
python3 - "$TMP/runtime-identity.json" <<'PY'
import json,re,sys
outer=json.load(open(sys.argv[1])); assert outer.get('ok') is True, outer
inner=json.loads(outer['stdout'])
assert inner.get('verified') is True, inner
assert inner.get('generation') == 18, inner
assert inner.get('release') == 'g18-host-commander', inner
assert re.fullmatch(r'[0-9a-f]{40}', inner.get('sourceSha','')), inner
print('MCP_G18_RUNTIME_IDENTITY=PASS source_sha='+inner['sourceSha'])
PY
...
for(const n of ["commander_open","commander_status","commander_runtime_identity","commander_file_read","commander_exec","commander_process_start","commander_docker_inspect","commander_storage_cleanup","commander_network_inspect"]) if(!names.includes(n)) throw new Error(`missing ${n}`);
