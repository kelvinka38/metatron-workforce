...
[[ "$EXPECTED_WORKFORCE_SHA" =~ ^[0-9a-f]{40}$ ]] || { echo 'MCP_G19_INVALID_WORKFORCE_SHA' >&2; exit 2; }

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNTIME_PATCH="$SCRIPT_DIR/patch-host-commander-runtime-g19.mjs"
BROKER_PATCH="$SCRIPT_DIR/patch-host-commander-broker-g19.py"
SUPERVISOR_SOURCE="$SCRIPT_DIR/commander-supervisor-g18.py"
SOURCE_TEST="$SCRIPT_DIR/test-host-commander-g19.py"
...
cleanup(){
  ...
  rm -f /tmp/metatron-commander/g19-acceptance.txt >/dev/null 2>&1 || true
  if [ "$code" -ne 0 ] && [ "$BROKER_INSTALLED" -eq 1 ] && [ "$RUNTIME_PROMOTED" -eq 0 ] && [ -n "$BROKER_BACKUP" ] && [ -f "$BROKER_BACKUP" ]; then
    ...
    echo 'MCP_G19_PREPROMOTION_BROKER_ROLLBACK_ATTEMPTED' >&2
  fi
  rm -rf "$TMP"
  trap - EXIT
  exit "$code"
...
if [ "$(id -u)" -ne 0 ]; then sudo -n true >/dev/null 2>&1 || { echo 'MCP_G19_ROOT_AUTHORITY_UNAVAILABLE' >&2; exit 3; }; fi

for required in "$RUNTIME_PATCH" "$BROKER_PATCH" "$SUPERVISOR_SOURCE" "$SOURCE_TEST"; do
  [ -r "$required" ] || { echo "MCP_G19_SOURCE_MISSING=$required" >&2; exit 4; }
done

GEN="$(current_generation)"
if [ "$GEN" -gt 19 ]; then
  echo "MCP_G19_REFUSE_NEWER_GENERATION generation=$GEN" >&2
  exit 5
fi
if [ "$GEN" -lt 18 ]; then
  echo "MCP_G19_UNEXPECTED_BASE_GENERATION generation=$GEN" >&2
  exit 5
fi

echo '=== SOURCE ACCEPTANCE ==='
python3 "$SOURCE_TEST"
echo 'MCP_G19_SOURCE_ACCEPTANCE=PASS'
...
[ "$RUNNING_SHA" = "$EXPECTED_WORKFORCE_SHA" ] || { echo "MCP_G19_WORKFORCE_SHA_NOT_READY expected=$EXPECTED_WORKFORCE_SHA actual=$RUNNING_SHA" >&2; exit 6; }
echo "MCP_G19_WORKFORCE_SHA_READY=$RUNNING_SHA"

echo '=== BUILD / VERIFY IMMUTABLE G19 RUNTIME ==='
if docker image inspect "$TARGET_IMAGE" >/dev/null 2>&1; then
  CID="$(docker create "$TARGET_IMAGE")"
  docker cp "$CID:/app/server.mjs" "$TMP/existing-server.mjs"
  docker rm -f "$CID" >/dev/null
  grep -q 'METATRON_HOST_COMMANDER_RUNTIME_G19' "$TMP/existing-server.mjs" || { echo 'MCP_G19_EXISTING_IMAGE_MARKER_MISSING' >&2; exit 7; }
  ...
  [ "$EXISTING_SOURCE_SHA" = "$EXPECTED_WORKFORCE_SHA" ] || { echo "MCP_G19_EXISTING_IMAGE_SOURCE_SHA_MISMATCH expected=$EXPECTED_WORKFORCE_SHA actual=$EXISTING_SOURCE_SHA" >&2; exit 7; }
  [ "$EXISTING_RELEASE" = g19-host-commander ] || { echo 'MCP_G19_EXISTING_IMAGE_RELEASE_LABEL_INVALID' >&2; exit 7; }
  echo "MCP_G19_IMAGE_ALREADY_PRESENT=PASS source_sha=$EXISTING_SOURCE_SHA"
else
  ...
  grep -q 'METATRON_HOST_COMMANDER_RUNTIME_G19' "$TMP/server.mjs"
  ...
  grep -q 'register("commander_file_remove"' "$TMP/server.mjs"
...
RUN node --check /app/server.mjs \
 && grep -q 'METATRON_HOST_COMMANDER_RUNTIME_G19' /app/server.mjs \
 && grep -q 'register("commander_open"' /app/server.mjs \
 && grep -q 'register("commander_runtime_identity"' /app/server.mjs \
 && grep -q 'register("commander_file_remove"' /app/server.mjs \
...
DOCKER
  docker build --pull=false \
    --label "metatron.mcp.source_sha=$EXPECTED_WORKFORCE_SHA" \
    --label "metatron.mcp.release=g19-host-commander" \
    -t "$TARGET_IMAGE" "$TMP"
  echo 'MCP_G19_IMAGE_BUILD=PASS'
fi
...
[ -n "$TARGET_IMAGE_ID" ] || { echo 'MCP_G19_IMAGE_ID_MISSING' >&2; exit 7; }
[ "$TARGET_SOURCE_SHA" = "$EXPECTED_WORKFORCE_SHA" ] || { echo "MCP_G19_IMAGE_SOURCE_SHA_MISMATCH expected=$EXPECTED_WORKFORCE_SHA actual=$TARGET_SOURCE_SHA" >&2; exit 7; }
[ "$TARGET_RELEASE" = g19-host-commander ] || { echo 'MCP_G19_IMAGE_RELEASE_LABEL_INVALID' >&2; exit 7; }
echo "MCP_G19_IMAGE_ID=$TARGET_IMAGE_ID source_sha=$TARGET_SOURCE_SHA release=$TARGET_RELEASE"

echo '=== PREPARE ATOMIC BROKER + SUPERVISOR INSTALL ==='
run_root cp "$BROKER" "$TMP/broker.current.py"
python3 "$BROKER_PATCH" "$TMP/broker.current.py" "$TMP/broker.g19.py"
python3 -m py_compile "$TMP/broker.g19.py" "$SUPERVISOR_SOURCE"
grep -q 'METATRON_HOST_COMMANDER_BROKER_G19' "$TMP/broker.g19.py"
grep -q 'def op_commander_open(a):' "$TMP/broker.g19.py"
grep -q 'def op_commander_runtime_identity(a):' "$TMP/broker.g19.py"
grep -q 'def op_commander_file_remove(a):' "$TMP/broker.g19.py"
grep -q 'def op_commander_storage_cleanup(a):' "$TMP/broker.g19.py"

BACKUP_DIR=/opt/metatron/ssh-mcp/backups
run_root mkdir -p "$BACKUP_DIR" /usr/local/lib
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
BROKER_BACKUP="$BACKUP_DIR/metatron-mcp-broker.pre-g19.$STAMP"
run_root cp "$BROKER" "$BROKER_BACKUP"
if run_root test -f "$SUPERVISOR_TARGET"; then
  SUPERVISOR_BACKUP="$BACKUP_DIR/metatron-commander-supervisor.pre-g19.$STAMP.py"
  run_root cp "$SUPERVISOR_TARGET" "$SUPERVISOR_BACKUP"
fi
run_root install -o root -g root -m 0755 "$TMP/broker.g19.py" "$BROKER"
run_root install -o root -g root -m 0755 "$SUPERVISOR_SOURCE" "$SUPERVISOR_TARGET"
BROKER_INSTALLED=1
run_root grep -q 'METATRON_HOST_COMMANDER_BROKER_G19' "$BROKER"
run_root grep -q 'commander-supervisor-g18.py' "$SUPERVISOR_TARGET"
echo "MCP_G19_BROKER_INSTALL=PASS backup=$BROKER_BACKUP"
...
printf '%s' "$server_probe" | grep -q '"ok": true\|"ok":true' || { echo 'MCP_G19_BROKER_BASELINE_PROBE_FAILED' >&2; exit 8; }

echo '=== BROKER COMMANDER ACCEPTANCE ==='
BINDING=0123456789abcdef0123456789abcdef
OPEN="$(broker_call "{\"op\":\"commander_open\",\"args\":{\"principal\":\"founder\",\"client_binding\":\"$BINDING\",\"purpose\":\"g19-production-acceptance\",\"scope\":\"maintenance\",\"ttl_seconds\":900}}")"
...
[ -n "$SESSION" ] && [ -n "$FENCE" ] || { echo 'MCP_G19_COMMANDER_SESSION_FAILED' >&2; exit 9; }
echo "MCP_G19_COMMANDER_SESSION=PASS session=$SESSION"
...
if broker_call "{\"op\":\"commander_open\",\"args\":{\"principal\":\"not-founder\",\"client_binding\":\"$BINDING\",\"purpose\":\"must-deny\"}}" > "$TMP/deny-founder.json" 2>&1; then
  echo 'MCP_G19_FOUNDER_GUARD_FAILED' >&2; exit 9
fi
...
printf '%s' "$EXEC" | grep -q 'uptime\|durationMillis' || { echo 'MCP_G19_EXEC_ACCEPTANCE_FAILED' >&2; exit 10; }

if broker_call "{\"op\":\"commander_file_read\",\"args\":{$auth,\"path\":\"/etc/shadow\"}}" > "$TMP/deny-secret.json" 2>&1; then
  echo 'MCP_G19_SECRET_GUARD_FAILED' >&2; exit 10
fi
...
if broker_call "{\"op\":\"commander_file_write\",\"args\":{$auth,\"path\":\"/opt/metatron/metatron-workforce/COMMANDER-MUST-NOT-WRITE\",\"content\":\"deny\"}}" > "$TMP/deny-source.json" 2>&1; then
  echo 'MCP_G19_SOURCE_GUARD_FAILED' >&2; exit 10
fi
grep -q 'commander_write_scope_denied\|commander_canonical_source_mutation_denied' "$TMP/deny-source.json"

WRITE="$(broker_call "{\"op\":\"commander_file_write\",\"args\":{$auth,\"path\":\"/tmp/metatron-commander/g19-acceptance.txt\",\"content\":\"alpha\"}}")"
printf '%s' "$WRITE" | grep -q '"ok": true\|"ok":true'
PATCH="$(broker_call "{\"op\":\"commander_file_patch\",\"args\":{$auth,\"path\":\"/tmp/metatron-commander/g19-acceptance.txt\",\"old_text\":\"alpha\",\"new_text\":\"beta\",\"expected_occurrences\":1}}")"
...
broker_call "{\"op\":\"commander_process_input\",\"args\":{$auth,\"process_id\":\"$PROCESS\",\"data\":\"hello-g19\\n\"}}" >/dev/null
PROCESS_OK=0
for _ in $(seq 1 80); do
  POUT="$(broker_call "{\"op\":\"commander_process_output\",\"args\":{$auth,\"process_id\":\"$PROCESS\",\"offset\":0,\"max_bytes\":65536}}")"
  if printf '%s' "$POUT" | grep -q 'hello-g19'; then PROCESS_OK=1; break; fi
  sleep 0.1
done
[ "$PROCESS_OK" -eq 1 ] || { echo 'MCP_G19_INTERACTIVE_OUTPUT_FAILED' >&2; exit 11; }
broker_call "{\"op\":\"commander_process_terminate\",\"args\":{$auth,\"process_id\":\"$PROCESS\"}}" >/dev/null
echo 'MCP_G19_INTERACTIVE_PROCESS_ACCEPTANCE=PASS'
...
if docker inspect "$FIXTURE" >/dev/null 2>&1; then echo 'MCP_G19_DOCKER_REMOVE_VERIFY_FAILED' >&2; exit 12; fi
echo 'MCP_G19_DOCKER_ACCEPTANCE=PASS'
...
echo 'MCP_G19_BROKER_COMMANDER_ACCEPTANCE=PASS'

if [ "$GEN" -eq 18 ]; then
  echo '=== FENCED G19 MCP PROMOTION ==='
...
[ "$(current_generation)" -eq 19 ] || { echo 'MCP_G19_PROMOTION_TIMEOUT' >&2; exit 13; }
RUNTIME_PROMOTED=1
ACTIVE="$(current_active)"
STANDBY="$(current_standby)"
[ "$ACTIVE" = metatron-mcp-runtime-g19 ] || { echo "MCP_G19_ACTIVE_UNEXPECTED=$ACTIVE" >&2; exit 13; }
[ "$STANDBY" = metatron-mcp-runtime-g19-standby ] || { echo "MCP_G19_STANDBY_UNEXPECTED=$STANDBY" >&2; exit 13; }
for name in "$ACTIVE" "$STANDBY"; do
  docker inspect "$name" --format '{{.State.Status}}/{{.State.Health.Status}}' | grep -qx 'running/healthy'
  docker exec "$name" grep -q 'METATRON_HOST_COMMANDER_RUNTIME_G19' /app/server.mjs
done
curl -fsS --max-time 10 https://ssh.metatron.vn/ready >/dev/null
echo 'MCP_G19_PROMOTION=PASS'

echo '=== RUNTIME IDENTITY ACCEPTANCE ==='
IDENTITY_OPEN="$(broker_call "{\"op\":\"commander_open\",\"args\":{\"principal\":\"founder\",\"client_binding\":\"$BINDING\",\"purpose\":\"g19-runtime-identity\",\"scope\":\"read_only\",\"ttl_seconds\":300}}")"
printf '%s' "$IDENTITY_OPEN" > "$TMP/identity-open.json"
read -r ID_SESSION ID_FENCE < <(python3 - "$TMP/identity-open.json" <<'PY'
import json,sys
outer=json.load(open(sys.argv[1])); assert outer.get('ok') is True, outer
inner=json.loads(outer['stdout']); print(inner['sessionId'],inner['fencingToken'])
PY
)
IDENTITY_AUTH="\"principal\":\"founder\",\"client_binding\":\"$BINDING\",\"session_id\":\"$ID_SESSION\",\"fencing_token\":\"$ID_FENCE\""
IDENTITY="$(broker_call "{\"op\":\"commander_runtime_identity\",\"args\":{$IDENTITY_AUTH}}")"
printf '%s' "$IDENTITY" > "$TMP/runtime-identity.json"
python3 - "$TMP/runtime-identity.json" "$EXPECTED_WORKFORCE_SHA" <<'PY'
import json,re,sys
outer=json.load(open(sys.argv[1])); assert outer.get('ok') is True, outer
inner=json.loads(outer['stdout'])
assert inner.get('verified') is True, inner
assert inner.get('generation') == 19, inner
assert inner.get('release') == 'g19-host-commander', inner
assert inner.get('sourceSha') == sys.argv[2], inner
print('MCP_G19_RUNTIME_IDENTITY=PASS source_sha='+inner['sourceSha'])
PY
broker_call "{\"op\":\"commander_close\",\"args\":{$IDENTITY_AUTH}}" >/dev/null
...
for(const n of ["commander_open","commander_status","commander_runtime_identity","commander_file_read","commander_file_remove","commander_exec","commander_process_start","commander_docker_inspect","commander_storage_cleanup","commander_network_inspect"]) if(!names.includes(n)) throw new Error(`missing ${n}`);
if(names.includes("commander_shell")) throw new Error("raw commander shell must not exist");
console.log(`MCP_G19_TOOLS_LIST=PASS tools=${names.length}`);'

run_root grep -q 'METATRON_HOST_COMMANDER_BROKER_G19' "$BROKER"
run_root test -x "$SUPERVISOR_TARGET"
echo "MCP_G19_COMPLETE active=$ACTIVE standby=$STANDBY image=$TARGET_IMAGE_ID workforce_sha=$EXPECTED_WORKFORCE_SHA broker_backup=$BROKER_BACKUP"
