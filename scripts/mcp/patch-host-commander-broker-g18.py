...
def _cm_terminate_session_processes(session):
    root = os.path.join(CM_ROOT, "processes", session.get("sessionId", ""))
    if not os.path.isdir(root):
        return 0
    count = 0
    for name in os.listdir(root)[:100]:
        if not CM_PROCESS_RE.fullmatch(name):
            continue
        pdir = os.path.join(root, name)
        if not os.path.isdir(pdir):
            continue
        try:
            fd = os.open(os.path.join(pdir, "control"), os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
            with os.fdopen(fd, "w", encoding="utf-8") as fh:
                fh.write("TERMINATE\n")
                fh.flush()
                os.fsync(fh.fileno())
...
def _cm_load_session(a, mutation=False):
    ...
    if float(session.get("expiresAt", 0)) <= time.time():
        terminated = _cm_terminate_session_processes(session)
        session["status"] = "EXPIRED"
        session["updatedAt"] = time.time()
        _cm_atomic_json(path, session)
        _cm_audit("session_expired", session, {"terminationRequests": terminated})
...
def _cm_mutation_lock(session):
    # One host-wide mutation fence is intentionally conservative. It prevents two
    # independently authorized Commander sessions from racing the same host target.
    lock_path = os.path.join(CM_ROOT, "host-mutation.lock")
...
def op_commander_close(a):
    ...
    _cm_audit("session_close", session, {"terminationRequests": terminated})
    return _cm_response({"accepted": True, "executed": True, "mutated": True, "verified": True, "sessionId": session["sessionId"], "status": "COMPLETED", "terminationRequests": terminated})


def op_commander_runtime_identity(a):
    check_keys(a, ["principal", "client_binding", "session_id", "fencing_token"], ["principal", "client_binding", "session_id", "fencing_token"])
    session = _cm_load_session(a, False)
    state_path = "/var/lib/metatron-mcp/router/router.json"
    try:
        with open(state_path, "r", encoding="utf-8") as fh:
            router = json.load(fh)
    except Exception:
        fail("commander_router_state_unavailable")
    active = str(router.get("active", ""))
    standby = str(router.get("standby", ""))
    generation = int(router.get("generation", 0))
    if not active:
        fail("commander_runtime_active_missing")
    rc, out, err = _hm_run(["docker", "inspect", active], 30)
    if rc != 0:
        fail("commander_runtime_inspect_failed")
    raw = json.loads(out)[0]
    image_tag = (raw.get("Config") or {}).get("Image", "")
    image_id = str(raw.get("Image", ""))
    labels = (raw.get("Config") or {}).get("Labels") or {}
    source_sha = str(labels.get("metatron.mcp.source_sha", ""))
    release = str(labels.get("metatron.mcp.release", ""))
    payload = {
        "accepted": True,
        "executed": True,
        "mutated": False,
        "verified": bool(generation >= 18 and source_sha and release == "g18-host-commander"),
        "generation": generation,
        "active": active,
        "standby": standby,
        "runtimeImage": image_tag,
        "imageId": image_id,
        "sourceSha": source_sha,
        "release": release,
        "brokerCapability": "g18-host-commander",
    }
    _cm_audit("runtime_identity", session, {"generation": generation, "active": active, "sourceSha": source_sha})
    return _cm_response(payload, payload["verified"], 0 if payload["verified"] else 1, "" if payload["verified"] else "commander_runtime_identity_unverified")
...
def op_commander_process_start(a):
    ...
    if not isinstance(timeout, int) or timeout < 5 or timeout > 1800:
        fail("commander_timeout_invalid")
    remaining = max(1, int(float(session.get("expiresAt", 0)) - time.time()))
    timeout = min(timeout, remaining)
    if timeout < 5:
        fail("commander_session_too_close_to_expiry")
