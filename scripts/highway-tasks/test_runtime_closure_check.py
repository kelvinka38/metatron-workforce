"""Tests for runtime_closure_check.py -- proves the fanout dispatch timing fixes without needing a
...
class RuntimeClosureCheckTest(unittest.TestCase):
    def test_elevated_spread_reproducing_incident_does_not_fail_closure(self):
        ...
        # dispatch_lag is kept comfortably under its own warn threshold so this test isolates the
        # spread behavior specifically.
    ...
    def test_elevated_dispatch_lag_reproducing_incident_does_not_fail_closure(self):
        # Recomputing the real incident's numbers showed dispatch_lag was ~3.4s (deploy finished at
        # 00:33:47.780266, production-highway-fast started at 00:33:51.190535) -- it would also have
        # failed under the old 2.0s hard bound had start_spread not failed first. start_spread is kept
        # well under its own warn threshold here to isolate the lag behavior specifically.
        build, parent = _build_and_deploy("2026-09-16T00:33:10.000000+00:00")
        wo = _task("hw-a", "work-observability", "SUCCEEDED", TARGET,
                   "2026-09-16T00:33:13.000000+00:00", "2026-09-16T00:33:14.500000+00:00")
        crf = _task("hw-b", "control-room-fast", "SUCCEEDED", TARGET,
                    "2026-09-16T00:33:13.100000+00:00", "2026-09-16T00:33:14.600000+00:00")
        phf = _task("hw-c", "production-highway-fast", "SUCCEEDED", TARGET,
                    "2026-09-16T00:33:13.400000+00:00", "2026-09-16T00:33:14.900000+00:00")
        get = _fake_get([build, parent, wo, crf, phf])
        run = _fake_run(TARGET, "healthy")
        ...
        self.assertIn("HIGHWAY_RUNTIME_CLOSURE=PASS", joined)
        self.assertIn("HIGHWAY_LIVE_EXACT_SHA=PASS", joined)
        self.assertIn("HIGHWAY_RUNTIME_CLOSURE_WARNING=fanout dispatch lag elevated", joined)
        self.assertIn("HIGHWAY_RUNTIME_CLOSURE_DISPATCH_LAG_SECONDS=3.400", joined)
        self.assertNotIn("HIGHWAY_RUNTIME_CLOSURE_WARNING=fanout dispatch spread elevated", joined)
    ...
    def test_lag_at_or_above_new_hard_cap_still_fails_closure(self):
        # Proves the raised lag threshold still catches a genuine defect, rather than simply
        # removing the check.
        build, parent = _build_and_deploy("2026-09-16T00:33:10.000000+00:00")
        wo = _task("hw-a", "work-observability", "SUCCEEDED", TARGET,
                   "2026-09-16T00:33:15.500000+00:00", "2026-09-16T00:33:17.000000+00:00")
        crf = _task("hw-b", "control-room-fast", "SUCCEEDED", TARGET,
                    "2026-09-16T00:33:15.600000+00:00", "2026-09-16T00:33:17.100000+00:00")
        phf = _task("hw-c", "production-highway-fast", "SUCCEEDED", TARGET,
                    "2026-09-16T00:33:15.700000+00:00", "2026-09-16T00:33:17.200000+00:00")
        # dispatch_lag = 15.7 - 10.0 = 5.7s -- above the new 5.0s hard cap; spread stays at 0.2s
        get = _fake_get([build, parent, wo, crf, phf])
        run = _fake_run(TARGET, "healthy")

        with self.assertRaisesRegex(AssertionError, "fanout dispatch lag too large"):
            rcc.run_closure(get, TARGET, PARENT_ID, run=run)
