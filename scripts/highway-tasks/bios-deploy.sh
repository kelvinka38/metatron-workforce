...
CI_JSON=$(curl -fsSL \
  ...
  "https://api.github.com/repos/kelvinka38/bios/actions/runs?head_sha=$BIOS_SHA&status=completed&per_page=20")
printf '%s' "$CI_JSON" | python3 -c 'import json,sys; d=json.load(sys.stdin); ok=any(r.get("name")=="BIOS Runtime" and r.get("head_sha")==sys.argv[1] and r.get("conclusion")=="success" for r in d.get("workflow_runs",[])); assert ok, "exact BIOS SHA has no successful BIOS Runtime CI"' "$BIOS_SHA"
echo "BIOS_EXACT_SHA_CI=PASS"
...
echo 'UNIT_TEST_REPLAY=SKIPPED_EXACT_SHA_CI_ALREADY_PASS'
