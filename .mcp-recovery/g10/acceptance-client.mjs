...
function mintSigned(payload) { const encoded=b64url(JSON.stringify(payload)); return `${encoded}.${hmac(encoded)}`; }
async function request(path, options={}) { const r=await fetch(BASE+path,options); const text=await r.text(); let body; try{body=JSON.parse(text);}catch{body=text;} return {r,body,text}; }
async function bootstrap() {
  ...
  const q=new URLSearchParams({response_type:'code',client_id:clientId,redirect_uri:redirectUri,scope:'mcp:tools offline_access',state:'g10-acceptance',code_challenge:challenge,code_challenge_method:'S256'});
  const ui=await request('/oauth/authorize?'+q.toString());
  if(ui.r.status!==200||!ui.text.includes('Metatron Authorization')||!ui.text.includes('Telegram')) throw new Error('authorization_ui_failed');
  ...
  process.stdout.write(JSON.stringify({clientId,redirectUri,accessToken:token.body.access_token,refreshToken:token.body.refresh_token,adminCookie,csrf,authorizationUi:true}));
}
async function mcp() {
  const token=process.argv[3], method=process.argv[4]||'tools/list';
  const payload=method==='initialize'?{jsonrpc:'2.0',id:1,method:'initialize',params:{protocolVersion:'2025-11-25',capabilities:{},clientInfo:{name:'g10-acceptance',version:'1'}}}:{jsonrpc:'2.0',id:2,method:'tools/list',params:{}};
...
async function revoke() {
  ...
  await r.arrayBuffer(); process.stdout.write(JSON.stringify({status:r.status}));
}
async function metadata() {
  const a=await request('/.well-known/oauth-authorization-server'),p=await request('/.well-known/oauth-protected-resource');
...
async function unauth() {
  const r=await fetch(BASE+'/mcp',{method:'POST',headers:{'content-type':'application/json','accept':'application/json, text/event-stream','mcp-protocol-version':'2025-11-25'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'tools/list',params:{}})}); await r.arrayBuffer();
  process.stdout.write(JSON.stringify({status:r.status,wwwAuthenticate:String(r.headers.get('www-authenticate')||'')}));
}
const mode=process.argv[2];
if(mode==='bootstrap') await bootstrap(); else if(mode==='mcp') await mcp(); else if(mode==='refresh') await refresh(); else if(mode==='revoke') await revoke(); else if(mode==='metadata') await metadata(); else if(mode==='unauth') await unauth(); else throw new Error('unknown_mode');
