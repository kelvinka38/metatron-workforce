...
cd "$BUILD"
echo "[START_RESILIENT_V2_2] $(date -Is)" > "$LOG"
...
public_transport_ok() {
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 6 \
    ...
    -H 'User-Agent: metatron-release-transport-probe/2.2' \
...
[ -s "$STATE/generation" ] || { echo '[V2_2_REQUIRES_MIGRATED_TOPOLOGY]' >> "$LOG"; exit 20; }
...
sync || true
if ! wait_resource_headroom; then
  echo '[RESOURCE_HEADROOM_TIMEOUT]' >> "$LOG"; exit 26
fi
...
echo "[DONE_RESILIENT_V2_2] generation=$next active=$candidate standby=$old_active $(date -Is)" >> "$LOG"
