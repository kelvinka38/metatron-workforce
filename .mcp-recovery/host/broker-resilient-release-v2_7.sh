...
cd "$BUILD"
echo "[START_RESILIENT_V2_7] $(date -Is)" > "$LOG"
...
public_transport_once() {
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 4 \
    ...
    -H 'MCP-Protocol-Version: 2024-11-05' -H 'User-Agent: metatron-release-transport-probe/2.7' \
...
fetch('http://127.0.0.1:3004/status',{signal:AbortSignal.timeout(2500)})
 .then(r=>r.json()).then(j=>process.stdout.write(String(Number(j.sessions?.[b]||0))))
 .catch(()=>process.exit(3));
NODE
}
max_session_count() {
  backend="$1"
  a=$(router_session_count metatron-mcp-router-a "$backend" || echo 999999)
  b=$(router_session_count metatron-mcp-router-b "$backend" || echo 999999)
  case "$a" in ''|*[!0-9]*) a=999999;; esac
  case "$b" in ''|*[!0-9]*) b=999999;; esac
  if [ "$a" -ge "$b" ]; then printf '%s' "$a"; else printf '%s' "$b"; fi
}
...
public_transport_hard_ok || { echo '[BASELINE_PUBLIC_TRANSPORT_FAILED]' >> "$LOG"; exit 27; }
router_transport_ok metatron-mcp-router-a || { echo '[BASELINE_ROUTER_A_FAILED]' >> "$LOG"; exit 28; }
router_transport_ok metatron-mcp-router-b || { echo '[BASELINE_ROUTER_B_FAILED]' >> "$LOG"; exit 29; }
...
if [ -n "$old_standby" ] && docker inspect "$old_standby" >/dev/null 2>&1; then
  sessions=$(max_session_count "$old_standby")
...
# Commit generation only after runtime + connector failover acceptance.
printf '%s\n' "$next" > "$STATE/generation"
chmod 600 "$STATE/generation" 2>/dev/null || true
write_state "$next" "$candidate" "$old_active" ""
fence || abort_release 99 '[FENCE_LOST_AT_COMMIT]'
public_transport_hard_ok || abort_release 56 '[FINAL_PUBLIC_TRANSPORT_FAILED]'
if [ -n "$old_standby" ] && [ "$old_standby" != "$old_active" ]; then docker rm -f "$old_standby" >/dev/null 2>&1 || true; fi
echo "DRAIN_POLICY=previous-active-retained-as-standby prior_standby_retired_only_when_session_count_zero" >> "$LOG"
echo "[DONE_RESILIENT_V2_7] generation=$next active=$candidate standby=$old_active artifact=$artifact_id $(date -Is)" >> "$LOG"
