...
public_transport_once() {
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 4 \
    ...
    -H 'MCP-Protocol-Version: 2024-11-05' -H 'User-Agent: metatron-release-transport-probe/3.3' \
...
# Runtime release must not destructively restart an unchanged Cloudflare Tunnel failure domain.
# Cloudflare replicas provide HA, but removing a replica can break connections and replica failover
# is not traffic-steering/load-balancer semantics. Destructive tunnel failover certification is a
# separate resilience lane. Runtime release verifies tunnel baseline + end-to-end transport only.
stable_running() {
...
public_transport_hard_ok || abort_release 55 '[TUNNEL_BASELINE_PUBLIC_TRANSPORT_FAILED]'
...
echo "[DONE_RESILIENT_V3_3] generation=$next active=$candidate standby=$standby_clone artifact=$artifact_id $(date -Is)" >> "$LOG"
