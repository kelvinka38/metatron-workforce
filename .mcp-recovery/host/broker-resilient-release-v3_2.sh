...
cd "$BUILD"
echo "[START_RESILIENT_V3_2] $(date -Is)" > "$LOG"
...
public_transport_once() {
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 4 \
    ...
    -H 'MCP-Protocol-Version: 2024-11-05' -H 'User-Agent: metatron-release-transport-probe/3.2' \
...
cutover_soft=$(count_soft_retries "$cutover_log")
cutover_failures=$(count_failures "$cutover_log")
echo "CUTOVER_SOFT_RETRIES=$cutover_soft" >> "$LOG"
...
# Hard-failure acceptance is meaningful only when the standby carries the same cheap-readiness
# semantics as the promoted active. Retire legacy old_active only after its pinned sessions drain.
standby_clone="${candidate}-standby"
old_active_sessions=$(host_session_count "$old_active")
echo "PREVIOUS_ACTIVE_SESSIONS_BEFORE_FAILOVER backend=$old_active count=$old_active_sessions" >> "$LOG"
[ "$old_active_sessions" -eq 0 ] || abort_release 62 '[PREVIOUS_ACTIVE_NOT_DRAINED_BEFORE_FAILOVER]'

docker stop -t 5 "$old_active" >> "$LOG" 2>&1 || true
docker rm -f "$standby_clone" >/dev/null 2>&1 || true
wait_resource_headroom 262144 60 || abort_release 63 '[PRE_FAILOVER_STANDBY_HEADROOM_FAILED]'
docker run -d --name "$standby_clone" --restart unless-stopped \
  --label metatron.mcp.role=runtime --label metatron.mcp.generation="$next" \
  --memory 512m --memory-swap 768m --pids-limit 256 \
  --network metatron-gateway-online --add-host host.docker.internal:host-gateway \
  -v /opt/metatron/ssh-mcp/id_ed25519:/ssh/id_ed25519:ro "$image" >> "$LOG" 2>&1
wait_health "$standby_clone" 90 || abort_release 64 '[PRE_FAILOVER_STANDBY_NOT_READY]'
stable_health "$standby_clone" 5 1 || abort_release 65 '[PRE_FAILOVER_STANDBY_UNSTABLE]'
docker exec "$standby_clone" node - <<'NODE' >/dev/null 2>&1 || abort_release 66 '[PRE_FAILOVER_STANDBY_FUNCTIONAL_FAILED]'
fetch('http://127.0.0.1:3002/mcp',{method:'POST',headers:{'content-type':'application/json','accept':'application/json, text/event-stream','mcp-protocol-version':'2024-11-05'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'tools/list',params:{}}),signal:AbortSignal.timeout(4000)})
  .then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(2));
NODE
...
force_probe_both "$candidate" || abort_release 67 '[PRE_FAILOVER_ACTIVE_PROBE_FAILED]'
force_probe_both "$standby_clone" || abort_release 68 '[PRE_FAILOVER_STANDBY_PROBE_FAILED]'
public_transport_hard_ok || abort_release 69 '[PRE_FAILOVER_PUBLIC_FAILED]'
echo "STANDBY_CONVERGENCE_BEFORE_FAILOVER_PASS active=$candidate standby=$standby_clone" >> "$LOG"
...
runtime_soft=$(count_soft_retries "$failover_log")
runtime_failures=$(count_failures "$failover_log")
echo "RUNTIME_FAILOVER_SOFT_RETRIES=$runtime_soft" >> "$LOG"
...
# Cloudflared needs several seconds to restore its full four-QUIC-connection set.
# Do not remove the second connector while the restarted primary is still converging.
sleep 8
public_transport_hard_ok || abort_release 71 '[PRIMARY_TUNNEL_RESTABILIZATION_FAILED]'
echo 'PRIMARY_TUNNEL_RESTABILIZATION_PASS wait_seconds=8' >> "$LOG"
...
sleep 8
public_transport_hard_ok || abort_release 72 '[HA_TUNNEL_RESTABILIZATION_FAILED]'
echo 'HA_TUNNEL_RESTABILIZATION_PASS wait_seconds=8' >> "$LOG"
wait_probe_window "$tunnel_pid"
tunnel_soft=$(count_soft_retries "$tunnel_log")
tunnel_failures=$(count_failures "$tunnel_log")
echo "TUNNEL_FAILOVER_SOFT_RETRIES=$tunnel_soft" >> "$LOG"
...
# Commit generation only after homogeneous runtime failover + connector failover acceptance.
printf '%s\n' "$next" > "$STATE/generation"
chmod 600 "$STATE/generation" 2>/dev/null || true
write_state "$next" "$candidate" "$standby_clone" ""
fence || abort_release 99 '[FENCE_LOST_AT_COMMIT]'
public_transport_hard_ok || abort_release 70 '[FINAL_PUBLIC_TRANSPORT_FAILED]'
...
echo "DRAIN_POLICY=homogeneous-cheap-readiness-pair prior_runtime_retired_only_when_host_session_count_zero" >> "$LOG"
echo "[DONE_RESILIENT_V3_2] generation=$next active=$candidate standby=$standby_clone artifact=$artifact_id $(date -Is)" >> "$LOG"
