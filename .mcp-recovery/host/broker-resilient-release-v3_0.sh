...
cd "$BUILD"
echo "[START_RESILIENT_V3_0] $(date -Is)" > "$LOG"
...
public_transport_once() {
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 4 \
    ...
    -H 'MCP-Protocol-Version: 2024-11-05' -H 'User-Agent: metatron-release-transport-probe/3.0' \
...
echo "[DONE_RESILIENT_V3_0] generation=$next active=$candidate standby=$standby_clone artifact=$artifact_id $(date -Is)" >> "$LOG"
